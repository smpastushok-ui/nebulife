...
Повний канонічний перелік ігрових модулів, їх статусів і ключових файлів — у [`GAME_MODULES.md`](GAME_MODULES.md). **Додавання нового модуля або суттєвої функції вимагає оновлення цього реєстру.**
...
**Платформа**: Mobile-only — Android + iOS (Capacitor), PixiJS v8 + React. Player-facing веб-версії немає; браузер використовується лише для розробки та внутрішніх admin-інструментів
...
- Обсерваторія шукає унікальні космічні події та об'єкти, не лише "штатні" сигнали.
- Шанс цільового сигналу за рівнем: L1 10%, L2 20%, L3 30%, L4 35%, L5 40%.
- 1 год: штатні/аномальні/рідкісні; 6 год: рідкісні + шанс епічних; 24 год: епічні/легендарні.
- 24-годинний пошук завжди щось повертає: якщо епічна/легендарна сигнатура не знайдена, гравець отримує штатну подію.
- Дублікати дозволені для re-roll/колекції, але є unseen bias і pity після серії дублікатів; темп розрахований приблизно на рік регулярного закриття каталогу.
- Орбітальний телескоп відкриває активне дослідження подій: до часу події показується таймер, після `eventTime` кнопка активна 24 години, потім подія стає неактивною. Без телескопа обсерваторія показує лише таймінг і досьє без зображення.
...
- **Згадки AI/ШІ у player-facing тексті** — заборонено (див. §1 «Без згадок AI»); пояснювати преміум через всесвіт гри (сканер, канал зйомки, архів), не через технологію бекенду
...
Гравець може грати на кількох мобільних пристроях (телефон + планшет) або перевстановити гру без втрати прогресу. Весь ігровий стан синхронізується через `game_state` JSONB колонку в таблиці `players`.
...
### 12.5 Surface / orbital access gates

- Поверхня або орбітальна сітка планети відкривається тільки після повного Tier 3 дослідження.
- Solid planets без прибулого `colony_ship` працюють у survey mode: відкриття hex-слотів і ручний збір ресурсів дозволені, будівництво заблоковане з поясненням.
- `colony_hub` і Colony Center доступні тільки після docking `colony_ship` на планеті, яка вже придатна (`habitability >= 0.3`) або повністю тераформована.
- `gas-giant` / `ice-giant` завжди orbital-only: леткі ресурси, `landing_pad` як orbital anchor і лише `orbital_collector` для будівництва.
- Якщо на цільовій планеті ще немає `landing_pad` / `spaceport`, вантажна логістика доставляє ресурси в орбітальний кеш цієї планети. Кеш прив'язаний до планети й використовується для першої приймальної інфраструктури; після побудови майданчика це стає звичайним локальним запасом.
