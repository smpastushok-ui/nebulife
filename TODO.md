...
- [ ] **Code-splitting клієнтського бандла** (липень 2026). Прод-бандл — один монолітний чанк 10.8 МБ (2.6 МБ gzip); це корінь чорного екрана при першому завантаженні (зараз прикрито статичним HTML-сплешем в `index.html`). План: розбити через `build.rollupOptions.output.manualChunks` у `vite.config.ts` — окремі чанки для three.js, Babylon.js, PixiJS + lazy-import важких сцен (UniverseEngine, BiosphereView, Raid/Arena) через `React.lazy`. Мета: initial chunk < 3 МБ, решта догружається за потреби. Обережно: Capacitor WebView + офлайн-кеш чанків, перевірити що lazy-чанки не ламають перший запуск без мережі. Оцінка: 2-3 дні + повний регрес бут-флоу на пристрої.
