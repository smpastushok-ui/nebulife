...
const checks = [
  ...
  assert('raid manual desktop fire is hold-to-fire', /keys\.has\('f'\)/.test(raidEngine) && !/autoFire\s*=\s*true/.test(raidEngine), 'RaidEngine gates player fire on the held F/Enter key instead of forced auto-fire.'),
  assert('raid uses unified Pixi engine (no Three.js raid path)', !/from 'three'/.test(raidEngine) && /pixi\.js/.test(raidEngine), 'RaidEngine.ts is PixiJS-only; ship GLBs are baked to sprite atlases via ShipSpriteBaker.'),
