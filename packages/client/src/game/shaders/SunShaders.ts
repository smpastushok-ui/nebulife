...
void main() {
  ...
  float q = fbm(p - t * 0.5);
  vec3 warpedP = p + vec3(q) * 1.5;
  float gran = fbm(warpedP * 4.0 + t * 2.0);
  float cells = fbm(warpedP * 1.2 - t);
  float noiseVal = smoothstep(-0.1, 0.7, mix(cells, gran, 0.4));
  ...
  finalColor += colorCore * pow(rim, 3.0) * 0.8;
...
void main() {
  ...
  float alphaFalloff = 1.0 - smoothstep(0.68, 1.0, dist);
  float t = time * 0.05;
  vec3 p = normalize(vPosition) * 4.0;
  float flares = pow(abs(fbm(p - t)), 3.0) * 8.0;
  float a = (flares + pow(1.0 - rim, 3.0)) * alphaFalloff * 0.5 * alpha;
  gl_FragColor = vec4(colorBright, a);
}
`;
