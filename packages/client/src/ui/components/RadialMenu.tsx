...
function buildButtons(props: {
  ...
  if (isResearched) {
    ...
    list.push({
      ...
      dim: premDim,
      isPremium: true,
      submenuLabel: hasPhoto
        ? <>{t('radial.view_panorama')}</>
        : (
          <>
            {t('radial.panorama_cost')}
            <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="#7bb8ff" strokeWidth="1.2" style={{ display: 'inline-block', verticalAlign: 'middle', marginLeft: 2 }}>
