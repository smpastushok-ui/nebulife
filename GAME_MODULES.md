...
| Biosphere creatures (3D) | Дослід з елементами (2–4 з 10, порядок = будова/покрив/акценти, опційно біом планети) → окремий технічний visual prompt → Image → Tripo GLB → Babylon scene, до 3 істот/планету; для кожної істоти одним паралельним Gemini text-call створюється валідований двомовний `lore` (опис, історія, характер, раціон, середовище/поведінка, розмір, вага, lifespan) з детермінованим fallback; 360° detail viewer має ізольований світлий studio-фон, решта UI лишається dark | в розробці | `Biosphere/BiosphereView.tsx`, `Biosphere/CreatureDetailModal.tsx`, `packages/core/src/game/creature-experiment.ts`, `packages/core/src/game/creature-lore.ts`, `api/creatures/*`, `040-creature-models.sql`, `045-creature-lore.sql` |
