...
**Дослідження космосу / каталог подій:**
- Обсерваторія також запускає пошук унікальних космічних подій і об'єктів для каталогу.
- Рівень обсерваторії задає базовий шанс цільового сигналу: L1 10%, L2 20%, L3 30%, L4 35%, L5 40%.
- 1 година: короткий пошук штатних, аномальних і рідкісних сигналів.
- 6 годин: основний пошук рідкісних сигналів, з шансом епічних відлунь.
- 24 години: пошук епічних і легендарних сигнатур; якщо цільова сигнатура не випала, гравець гарантовано отримує штатну подію.
- Дублікати дозволені, але вибір зміщується до ще не знайдених записів; після кількох дублікатів спрацьовує pity-правило. Баланс розрахований так, щоб каталог із поточного набору подій вистачав приблизно на рік регулярних пошуків.
- Орбітальний телескоп перетворює таймінг події на активне дослідження: до `eventTime` кнопка показує countdown, після настання події доступне 24-годинне вікно дослідження, потім подія стає неактивною. Обсерваторія без телескопа показує лише досьє/час без зображення.
...
- `surface_landing` з surface payload + rover dropcraft відкриває Tier 3 для solid planets: життя, детальні ресурси, surface report і підтвердження колонізації.
- Для `gas-giant` / `ice-giant` Tier 3 замінюється `deep_atmosphere_probe` з atmosphere payload + carrier: глибокі шари атмосфери, storms і magnetic science без surface deposits.
- Запуск дослідницьких місій не витрачає minerals / volatiles / isotopes / water / research data. Ресурси витрачаються на виробництво готових payload/carrier. `landing_pad` не є вимогою запуску; він потрібен для логістики, доставки ресурсів і виробництва кораблів/вантажів на колонії.
...
- Поверхневі або орбітальні операції відкриваються тільки після повного Tier 3 дослідження планети.
- Solid planets після Tier 3 входять у survey mode: можна відкривати гекси й збирати локальні ресурси, але будівництво заблоковане до прибуття `colony_ship`.
- `colony_hub` і Colony Center з'являються тільки коли колонізаційний корабель прибув, а планета вже достатньо придатна (`habitability >= 0.3`) або тераформування завершено.
- `gas-giant` / `ice-giant` не вимагають colony ship: вони залишаються orbital-only, дозволяють відкриття гексів, збір летких речовин і будівництво лише `orbital_collector`.
- До появи `landing_pad` / `spaceport` вантажні рейси можуть доставляти ресурси тільки в орбітальний кеш планети. Це вирішує перший логістичний крок без "телепортації" на поверхню: кеш прив'язаний до планети й використовується для стартової приймальної інфраструктури.
- Ресурсний модуль на поверхні показує локальний склад планети: добуто / ліміт для minerals, volatiles, isotopes і water. Розділені елементи з Quantum Separator показуються в цьому модулі лише після відкриття періодичної таблиці (L48).
- Запаси ресурсів на solid planets мають економічний floor відносно орієнтовної вартості тераформування: у середньому близько +50% до витрат, але з seed-based розкидом між планетами та групами ресурсів. Це робить вибір цілі важливим: одна планета може бути багатою водою, інша леткими речовинами або ізотопами, а частина світів лишається посередньою.
...
- Геномний конструктор у Research Lab — доступний до 4 разів на день: кожен з 4 типів Spark of Life має власний незалежний кулдаун, що скидається о 00:00 UTC.
- Успішне складання одного типу іскри сьогодні блокує лише цей тип до наступного скидання; інші 3 типи лишаються доступними того ж дня.
